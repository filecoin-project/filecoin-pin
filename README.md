...
On Filecoin mainnet only, the experimental, explicit opt-in acquisition can bring missing FIL and USDFC from the same owner address on a selected EVM source network. `payments setup --auto` and `payments fund` share the same catalog-verified source contract; it is intentionally an allowlist, not a generic bridge, and the funded mainnet smoke test remains pending:
...
For both commands, the selected source networks are Filecoin, Arbitrum, Ethereum, Polygon, Avalanche, and BNB Chain. Base and Optimism are excluded because their L1 data fee cannot be capped by the transaction, so they cannot satisfy the command's hard source-native spend ceiling. A checkpoint created for an already-broadcast Base or Optimism route by an earlier version can still be polled to completion, but recovery never quotes or signs another transaction on those chains. `--from-token` accepts a unique symbol, an exact token address, or `native`; Filecoin Pin resolves it against the live [Squid](documentation/glossary.md#squid) catalog, verifies it on the selected chain, and proceeds only with a valid route. The source and Filecoin wallet must be controlled by the same private key. Keep the selected source chain's native gas available for exact-amount ERC-20 approvals and route execution. `SOURCE_RPC_URL` must point to the selected `--from-chain`, including Filecoin when `--from-chain filecoin` is selected; set it and `SQUID_INTEGRATOR_ID` in the environment rather than command history. The examples above use `--network` with the default Filecoin endpoint. To use a custom Filecoin endpoint, omit `--network` and set `RPC_URL` instead; never set both selectors. Never put a wallet key, provider credential, or credential-bearing RPC URL in a script, issue, or smoke-test report. Calibration and devnet intentionally fail acquisition closed: fund those wallets directly and retry. If Squid is unavailable or lacks a viable route, fund FIL and USDFC directly on Filecoin and rerun without `--from-*`; do not widen the source selection or spending cap automatically. If an acquisition completed but a later Filecoin Pay step failed, rerun only the direct Filecoin deposit or command shown by the CLI; do not repeat source spending.
