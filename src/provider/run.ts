...
export async function runProviderList(
  ...
  try {
    ...
    if (options.all) {
      ...
      spinner.message(
        `Fetching details for ${approvedIds.length} approved providers...`,
      );
      const providersOrNull = await Promise.all(
        approvedIds.map((id: number) => spRegistry.getProvider(id)),
      );
...
export async function runProviderShow(
  providerIdOrAddr: string,
  options: ProviderShowOptions,
): Promise<void> {
  const spinner = createSpinner();
  ...
  try {
    ...
    if (!Number.isNaN(id) && id.toString() === providerIdOrAddr) {
      ...
      spinner.stop(
        pc.yellow(
          "Querying by address is not directly supported, trying as ID if numeric.",
        ),
      );
    ...
    if (!provider) {
      spinner.stop(
        pc.red(`Provider ${providerIdOrAddr} not found or invalid.`),
      );
...
export async function runProviderPing(
  ...
  try {
    ...
    for (const p of providersToPing) {
      ...
      if (!serviceUrl) {
        console.log(
          `${pc.yellow("⚠")} ${p.name || p.id} [${p.serviceProvider}]: ${pc.gray("No PDP Service URL")}`,
        );
        continue;
      }
...
function printProvider(p: any) {
  ...
  if (data) {
    if (data.minPieceSizeInBytes != null)
      console.log(
        `  Min Piece Size: ${formatFileSize(data.minPieceSizeInBytes)}`,
      );
    if (data.maxPieceSizeInBytes != null)
      console.log(
        `  Max Piece Size: ${formatFileSize(data.maxPieceSizeInBytes)}`,
      );
    if (data.storagePricePerTibPerDay != null)
      console.log(
        `  Storage Price: ${formatUSDFC(BigInt(data.storagePricePerTibPerDay))} USDFC/TiB/Day`,
      );
    if (data.minProvingPeriodInEpochs != null)
      console.log(
        `  Min Proving Period: ${data.minProvingPeriodInEpochs} epochs`,
      );
  }
}
