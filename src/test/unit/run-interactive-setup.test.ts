...
const { mockPassword, mockIsCancel, mockIsTTY, mockParseCLIAuth, mockAssertOwnerAuth, mockInitializeSynapse } =
  vi.hoisted(() => ({
    mockPassword: vi.fn(),
    mockIsCancel: vi.fn(() => false),
    mockIsTTY: vi.fn(() => true),
    mockParseCLIAuth: vi.fn((options: { walletAddress?: string; sessionKey?: string; privateKey?: string }) => ({
      ...(options.privateKey != null ? { privateKey: options.privateKey } : {}),
      ...(options.walletAddress != null ? { walletAddress: options.walletAddress } : {}),
      ...(options.sessionKey != null ? { sessionKey: options.sessionKey } : {}),
    })),
    mockAssertOwnerAuth: vi.fn(),
...
vi.mock('../../utils/cli-auth.js', () => ({ parseCLIAuth: mockParseCLIAuth, assertOwnerAuth: mockAssertOwnerAuth }))
...
describe('runInteractiveSetup exit codes', () => {
  ...
  it('refuses a session key before any payment work (setup moves funds, owner wallet only)', async () => {
    mockAssertOwnerAuth.mockImplementationOnce(() => {
      throw new Error("payments setup needs the account owner's wallet; session keys can't move funds.")
    })
    ...
    expect(mockAssertOwnerAuth).toHaveBeenCalledWith(
      expect.objectContaining({ walletAddress: '0xowner', sessionKey: '0xsess' }),
      'payments setup'
    )
    expect(mockInitializeSynapse).not.toHaveBeenCalled()
  })
