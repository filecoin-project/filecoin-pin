...
export async function runAdd(options: AddOptions): Promise<AddResult> {
  ...
  try {
    ...
    const sizeDisplay = isDirectory
      ? ""
      : ` (${formatFileSize(pathStat.size)})`;
    ...
    spinner.stop(
      `${pc.green("✓")} Connected to ${pc.bold(synapse.chain.name)}`,
    );
    ...
    spinner.stop(
      `${pc.green("✓")} ${isDirectory ? "Directory" : "File"} packed with root CID: ${rootCid.toString()}`,
    );
    ...
    spinner.stop(
      `${pc.green("✓")} IPFS content loaded (${formatFileSize(carSize)})`,
    );
    ...
    const storageContextOptions: CreateStorageContextOptions = {
      ...
      callbacks: {
        onProviderSelected: (provider) => {
          spinner.message(
            `Connecting to storage provider: ${provider.name || provider.serviceProvider}...`,
          );
        },
    ...
    spinner.stop(
      `${pc.red("✗")} Add failed: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
